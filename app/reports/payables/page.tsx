...
export default function PayablesReportPage() {
  ...
  const fetchReport = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/reports/payables-aging');
  ...
  const exportToCSV = () => {
    ...
    let csv = 'Supplier,Total Outstanding,0-30 Days,30-60 Days,60+ Days\n';
    report.items.forEach((item) => {
      csv += `"${item.supplier_name}",${item.total_outstanding},${item.days_0_30},${item.days_30_60},${item.days_60_plus}\n`;
    });
    ...
    a.href = url;
    a.download = `payables-aging-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">📊 Payables Aging Report</h1>
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y">
            {report.items.map((item) => (
              <tr key={item.supplier_name} className="hover:bg-gray-50">
                <td className="px-6 py-4 font-semibold">{item.supplier_name}</td>
