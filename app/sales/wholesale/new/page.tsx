...
export default function NewSaleWholesalePage() {
  ...
  const handleSelectItem = (storeItem: ItemStoreStock) => {
    if (selectedItemIndex >= 0) {
      ...
      newItems[selectedItemIndex] = {
        ...
        item_code: item?.code || '',
        item_name: item?.name || '',
        unit_price: item?.wholesale_price || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/sales/wholesale?success=${result.data.invoice_number}`);
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold mt-2">New Wholesale Sale</h1>
        <p className="text-gray-600 text-sm mt-1">
          Create a new wholesale invoice with bulk pricing
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div className="border-t pt-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-lg font-semibold">Sale Items (Wholesale Pricing)</h2>
              {storeItems.length > 0 && (
